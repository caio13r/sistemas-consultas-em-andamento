...
def assign_permission_to_role(
    ...
    admin_only = ['view_users', 'create_users', 'edit_users', 'delete_users']
    if permission.name in admin_only and role.name != 'Administrador':
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Permissões de gerenciamento de usuários são exclusivas do perfil Administrador"
        )
