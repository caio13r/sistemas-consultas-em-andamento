...
from ..core.auth import get_password_hash, get_current_active_user, check_permission, get_user_permissions, get_user_roles
...
def get_users(
    ...
    current_user: User = Depends(check_permission("view_users"))
):
...
def update_user_by_admin(
    ...
    db: Session = Depends(get_db), 
    current_user: User = Depends(check_permission("edit_users"))
):
...
def delete_user(
    user_id: int, 
    db: Session = Depends(get_db), 
    current_user: User = Depends(check_permission("delete_users"))
):
    db_user = db.query(User).filter(User.id == user_id).first()
    if not db_user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
