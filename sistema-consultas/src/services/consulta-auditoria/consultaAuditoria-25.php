...
if (Session::get('grupo') != 0 && isset($row['CA25acesso']) && $row['CA25acesso'] == false) {
...
$tituloConsulta = 'Auditoria - Profissionais e Empresas em atividade sem a respectiva data de inscrição no CFO';
?>

<div class="col-md-6 offset-md-3 mb-4">
<h6 class="mb-2">Consultar profissionais e empresas em atividade sem a respectiva data de inscrição no CFO</h6>
        ...
        <div class="form-row">
            <div class="form-group col-md-12">
                ...
                <select id="cro" name="cro" class="form-control">
                    ...
                    <?php
                        if (Session::get('grupo') === 0 || (isset($row['CA25select']) && $row['CA25select'] == true)) {
    ...
    if (!empty($erro)) {
        ...
        try {
            ...
            $query = "SELECT
                        ...
                        Situação,
                        Detalhe,
                        [Data de inscrição] AS 'Data Regional',
                        [Data Início Situação]
                      FROM CFO_CWS.dbo.vw_Cons_Profissionais_Empresas_Sem_Data_Registro_Federal
                      {$whereClause}";
            ...
            error_log("Erro em auditoria 25: " . $error->getMessage());
...
<div class="row mt-4">
    <div class="col table-responsive">
        <table id="tabelaAuditoria25" class="table table-sm table-bordered table-striped table-hover mt-4 mb-4">
            <thead>
            <tr>
                <th scope="col">CRO</th>
            ...
            <tr>
                ...
                <td><?= htmlspecialchars($linha['Tipo de Inscrição'] ?? '') ?></td>
                <td><?= htmlspecialchars($linha['Situação'] ?? '') ?></td>
                <td><?= htmlspecialchars($linha['Detalhe'] ?? '') ?></td>
                <td><?= htmlspecialchars($linha['Data Regional'] ?? '') ?></td>
                <td><?= htmlspecialchars($linha['Data Início Situação'] ?? '') ?></td>
            </tr>
            <?php } ?>
            </tbody>
        </table>
