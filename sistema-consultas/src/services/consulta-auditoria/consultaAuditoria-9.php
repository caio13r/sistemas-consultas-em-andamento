...
if (Session::get('grupo') != 0 && (!isset($row['CA9acesso']) || $row['CA9acesso'] == false)) {
        ...
        <div class="form-row">
            <div class="form-group col-md-12">
                ...
                <select id="cro" name="cro" class="form-control">
                    ...
                    <?php
                      if (Session::get('grupo') === 0 || (isset($row['CA9select']) && $row['CA9select'] == true)) {
...
<?php if (isset($inputPost["submit"])) {
    ...
    if (!empty($erro)) {
        ...
        try {
            ...
            $query = "SELECT * FROM CFO_CWS.dbo.vw_Cons_RT_Em_Mais_De_Uma_Empresa" . $whereClause;
            ...
            error_log("Erro em auditoria 9: " . $error->getMessage());
        ...
        <table id="tabelaAuditoria9" class="table table-sm table-bordered table-striped table-hover mt-4 mb-4">
            ...
            <?php foreach ($result as $linha) : ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($linha['PJ_CRO'] ?? '') ?></td>
                    <td><?= htmlspecialchars($linha['PJ_Categoria'] ?? '') ?></td>
                    <td><?= htmlspecialchars($linha['PJ_Inscricao'] ?? '') ?></td>
                    <td><?= htmlspecialchars($linha['PJ_Razao_Social'] ?? '') ?></td>
                    <td><?= htmlspecialchars($linha['PJ_CNPJ'] ?? '') ?></td>
                    <td><?= htmlspecialchars($linha['PJ_Tipo_Inscricao'] ?? '') ?></td>
                    <td><?= htmlspecialchars($linha['PJ_Situacao'] ?? '') ?></td>
                    <td><?= htmlspecialchars($linha['PJ_Detalhe'] ?? '') ?></td>
                    <td><?= htmlspecialchars($linha['PJ_Natureza_Juridica'] ?? '') ?></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
