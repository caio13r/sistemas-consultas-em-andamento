...
try {
  ...
  // Inicialize arrays para contar a quantidade de resultados por tipo
  $quantidadePorTipo = [];

  // Iterar pelos resultados e contar a quantidade por tipo
  foreach ($result as $row) {
    $tipo = $row['tipo'];
    $quantidade = $row['quantidade'];

    $quantidadePorTipo[$tipo] = $quantidade;
  }
...
<script>
  ...
  var myChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: <?= json_encode(array_keys($quantidadePorTipo)) ?>,
      datasets: [{
        label: 'Quantidade',
        backgroundColor: 'rgba(54, 162, 235, 0.2)',
        borderColor: 'rgba(54, 162, 235, 1)',
        borderWidth: 1,
        data: <?= json_encode(array_values($quantidadePorTipo)) ?>
      }]
    },
    options: {
      responsive: true,
