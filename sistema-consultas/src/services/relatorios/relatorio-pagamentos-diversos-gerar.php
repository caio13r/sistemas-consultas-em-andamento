...
try {
    ...
    error_log("relatorio-pagamentos-diversos-gerar: " . $e->getMessage());
...
$headers = ['CRO', 'Forma_Pagamento', 'Total_Pagamento', 'Total_Arrecadado'];

$spreadsheet = new Spreadsheet();
$sheet = $spreadsheet->getActiveSheet();

// Título com data de emissão
$titulo = 'Relatório de Pagamentos Diversos - Período: ' . date('d/m/Y', strtotime($dataInicio)) . ' a ' . date('d/m/Y', strtotime($dataFim)) . ' - Emitido em: ' . date('d/m/Y H:i:s');
...
foreach ($dadosAgrupados as $cro => $dadosCro) {
    ...
    foreach ($dadosCro as $row) {
        ...
        $sheet->setCellValue('B' . $rowNum, $row['Forma_Pagamento']);
...
$fileName = 'Relatorio_Pagamentos_Diversos_' . date('Ymd_His') . '.xlsx';
