...
if (Session::get('grupo') != 0 && $row['DA4acesso'] == false) {
...
$tituloConsulta = 'Dados Abertos - Balanço Contábil';
?>
...
<div class="col-md-6 offset-md-3 mb-4">
    ...
    <form action="" method="post">
        ...
        <button type="submit" name="submit" class="btn btn-primary">Consultar Balanço Contábil</button>
    </form>
</div>
...
if (isset($_POST["submit"])) {
    try {
        ...
        $url = "https://cfo-br.implanta.net.br/portaltransparencia/servico/api/Balancete?referenciaInicio="
            . urlencode($dtInicio)
            . "&referenciaTermino="
            . urlencode($dtTermino)
        ;
        ...
        if (!$result['message']) {
            foreach ($result as &$item) {
                $item['analitica'] = $item['analitica'] ? "true" : "false";
                $item['valorAnterior'] = number_format($item['valorAnterior'], 2, ',', '.');
                $item['valorCredito'] = number_format($item['valorCredito'], 2, ',', '.');
                $item['valorDebito'] = number_format($item['valorDebito'], 2, ',', '.');
...
<?php } else { ?>
    ...
    <div class="row mt-4">
        <div class="col">
            <!-- Tabela de Balanço Contábil -->
            <h3>Balanço Contábil</h3>
            <table id="tabelaDados" class="table table-sm table-bordered table-striped table-hover mt-4 mb-4">
                ...
                <tbody>
                    ...
                    foreach ($result as $row) {
                        ...
                        echo "<td>" . $row['analitica'] . "</td>";
                        echo "<td>" . $row['valorAnterior'] . "</td>";
                        echo "<td>" . $row['valorCredito'] . "</td>";
                        echo "<td>" . $row['valorDebito'] . "</td>";
