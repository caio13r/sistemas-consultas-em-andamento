...
<div class="container-fluid">
    <div class="card">
        <div class="card-header">
            <h5><i class="fas fa-search-plus mr-2 mt-2"></i>Consulta de Auditoria</h5>
        </div>
        ...
        <div class="card-body">
                        ...
                        <?php foreach ($chunk as $label): ?>
                            <?php if ($label['disabled'] == 0): ?>
                                <?php if (Session::get('grupo') === 0 || $row['CA' . $label['referencial'] . 'acesso'] == true): ?>
                                    <a href="/consulta-auditoria?tipoConsulta=<?= $label['referencial'] ?>" class="btn-container">
                                        <button 
                                            type="button" 
            ...
            <?php
                if (isset($inputGet['tipoConsulta'])) {
                    require_once SERVICES_PATH . '/consulta-auditoria/consultaAuditoria-' . $_GET['tipoConsulta'] . '.php';
                }
            ?>
        </div>
    </div>
