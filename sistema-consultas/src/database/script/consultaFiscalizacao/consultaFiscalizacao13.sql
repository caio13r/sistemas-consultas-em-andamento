...
FROM (
    ...
    OUTER APPLY (
        SELECT
            ...
            MAX(CASE WHEN ReSit.Nome IN ('DESATIVADO', 'CANCELADO', 'PRÉ-CADASTRO') THEN ReReSit.DataInicioSituacao END) AS [Data_Desativado]
    ...
    INNER JOIN (
        SELECT
            ...
            COUNT(IIF(VFP.Nome IN ('Proativo'), 1, NULL)) AS [Proativa],
            COUNT(IIF(VFP.Nome IN ('Reativo'), 1, NULL)) AS [Reativa],
        ...
        GROUP BY ROLLUP(ReCat.Sigla, VFP.Fiscal)
    ) AS F
        ON A.Categoria = F.Categoria
WHERE A.Categoria LIKE ':categoria'
ORDER BY
    CASE
        WHEN F.Fiscal = 'TODOS' THEN 2
        WHEN F.Fiscal = 'Desconhecido' THEN 1
        ELSE 0
    END,
    F.Fiscal,
